...
from vestigia.prompts.hiking_choice import CATEGORIES, PARSE_ERROR, PROMPT


@pytest.mark.parametrize("response", CATEGORIES)
def test_hiking_choice_parser_accepts_each_exact_category(response: str) -> None:
...
def test_hiking_choice_parser_maps_every_invalid_response_to_parse_error(response: str) -> None:
    parsed = PROMPT.parser(response)
...
def test_hiking_choice_prompt_requires_one_ascii_category_only() -> None:
    assert PROMPT.id == "hiking_choice"
